...
TIMO MOD patch packages target an app by its stable bundle identifier. They never store the app-container UUID, because that UUID changes across devices and reinstalls.
...
4. Tap **Done**. TIMO MOD creates the project and its editable workspace.
5. Open **Files → TIMO MOD Workspace → Patches → project name → bundle ID**.
...
3. TIMO MOD captures the stable bundle identifier and relative path automatically and opens a patch draft.
4. Save the draft, open its workspace, and replace or rearrange the captured content as needed.

## Apply and restore

- **Apply** first synchronizes the workspace into the encrypted `.timo` package, resolves the current container from the bundle identifier, and validates every destination path.
- Existing targets are backed up before any replacement is written. Missing targets are created.
- Writes are journaled and verified. If applying fails partway through, TIMO MOD attempts to roll the transaction back.
...
- **Export** synchronizes the latest workspace contents before sharing the `.timo` file.
- Import from Files by opening or sharing a `.timo` package to TIMO MOD.
- A website may open the app with `timo://import?url=<percent-encoded HTTPS URL>`. Only HTTPS package URLs without embedded credentials are accepted.
- On a new device or installation, a protected package asks for its password once. TIMO MOD stores the unlocked content key in Keychain; the exported package remains encrypted and tied to its original password.
